#################################################################################
# FILENAME    : MIDIR_analysis_simgbda_rtb.R
# AUTHOR      : Jitong Lou <lou_jitong@lilly.com>
# DATE        : 06/10/2024
# DESCRIPTION : Proposed direct estimation method for RTB imputation, tested on simulated data based on the real data application in the article
#################################################################################
...
gbdadata_complete=tidyr::complete(gbdalong1, id, time_order, fill=list(miss=1)) %>% group_by(id) %>% 
  ...
  rename(value_raw_rtb=value_raw, value_raw_rtb_chg=value_raw_chg, value_rtb_chg=value_raw_chg, value_rtb=value_raw) %>%
...
result_theta = calc_est_RTB_theta_v3(
...
# Output contour plot 
gbda_rtb_tip=ggplot(data, aes(x = delta_t_v, y = delta_c_v, z = z_v)) +
