#################################################################################
# FILENAME    : MIDIR_analysis_simgbda_rd.R
# AUTHOR      : Jitong Lou <lou_jitong@lilly.com>
# DATE        : 06/10/2024
# DESCRIPTION : Proposed direct estimation method for RD imputation, tested on simulated data based on the real data application in the article
#################################################################################
...
data_temp=tidyr::complete(gbdalong1, id, time_order, fill=list(miss=1)) %>% group_by(id) %>% 
  mutate(base = rep(mean(base, na.rm = TRUE), length(base)), TRTSORT2 = rep(mean(TRTSORT2, na.rm = TRUE), length(TRTSORT2))) %>%
  rename(value_raw_rd=value_raw, adherence=ONTRTFLAG) %>%
  mutate(time=time_order) %>% # time for formula, time_order for waves argument
  mutate(subject=id)  # subject for id argument

gbdadata_complete = data_temp %>%
  inner_join(
    data_temp %>%
      group_by(subject) %>%
      summarise(last_adherence = last(adherence)),
    by = "subject"
  ) %>%
  mutate(
    value_rd = value_raw_rd * ifelse(miss == 1, NA, 1),
    value_ad_rd = value_raw_rd * ifelse(last_adherence == 1, 1, NA) * ifelse(miss == 1, NA, 1)
  ) %>%
  mutate(
    value_raw_rd_chg = value_raw_rd - base,
    value_rd_chg = value_rd - base,
    value_ad_rd_chg = value_ad_rd - base
  )
...
result_beta = mapply(
  FUN = fit_data_RD_beta,
  data = df_fit_tf_list, N = N_list, 
  MoreArgs = list(K = K, beta_family = gaussian(), beta_corstr = "unstructured", eps = 0),
  SIMPLIFY = FALSE
...
result_betaRD = mapply( 
  FUN = fit_data_RD_betaRD,
  data = df_fit_tf_list,
  MoreArgs = list(K = K),
  SIMPLIFY = FALSE
) # Regression model for RDs for each group
sapply(result_betaRD, function(x){
  range(rowSums(x$U))
}) # value of estimating equations, should be close to 0
...
result_tau = mapply(
  ...
  MoreArgs = list(K = K),
  SIMPLIFY = FALSE
) # proportion of adherent but missing outcome for each group
sapply(result_tau, function(x){
  range(sum(x$U))
}) # value of estimating equations, should be close to 0

result_tauAD = mapply(
  FUN = fit_data_RD_tauAD,
  data = df_fit_tf_list,
  MoreArgs = list(K = K),
  SIMPLIFY = FALSE
) # proportion of adherent and complete outcome for each group
sapply(result_tauAD, function(x){
  range(sum(x$U))
}) # value of estimating equations, should be close to 0

result_nuNAD = mapply(
  FUN = fit_data_RD_nuNAD,
  data = df_fit_tf_list,
  MoreArgs = list(K = K),
  SIMPLIFY = FALSE
) # mean of baseline for non-adherers in each group
sapply(result_nuNAD, function(x){
  range(sum(x$U))
}) # value of estimating equations, should be close to 0
...
result_delta = mapply( 
  ...
  beta_result = result_beta, betaRD_result = result_betaRD, 
  pi_result = result_pi, tau_result = result_tau, tauAD_result = result_tauAD,
  nuNAD_result = result_nuNAD, nu_result = result_nu,
  SIMPLIFY = FALSE
...
result_theta = calc_est_RD_theta_v2(
  delta_result_list = result_delta, l_delta = l_delta, mean_base, var_base
...
result_tip = cbind(
  ...
  calc_est_RD_TIP_v2(
    ...
    tip = TRUE, tip_delta, complete = FALSE)
...
# Output contour plot 
gbda_rd_tip=ggplot(data, aes(x = delta_t_v, y = delta_c_v, z = z_v)) +
