#################################################################################
# FILENAME    : MIDIR_analysis_simgbda_j2r.R
# AUTHOR      : Jitong Lou <lou_jitong@lilly.com>
# DATE        : 06/10/2024
# DESCRIPTION : Proposed direct estimation method for J2R imputation, tested on simulated data based on the real data application in the article
#################################################################################
...
gbdadata_complete=tidyr::complete(gbdalong1, id, time_order, fill=list(miss=1)) %>% group_by(id) %>% 
  ...
  rename(value_raw_j2r=value_raw, value_raw_j2r_chg=value_raw_chg, value_j2r_chg=value_raw_chg, value_j2r=value_raw) %>%
...
result_nuREF = fit_data_J2R_nu(
  data = df_fit_tf_list$control,
  K = K
) # mean of baseline for subjects in reference group
range(sum(result_nuREF$U)) # value of estimating equations, should be close to 0
...
result_delta = mapply( 
  FUN = calc_est_J2R_delta_v8,
  beta_result = result_beta, pi_result = result_pi, nu_result = result_nu, 
  reference = as.list(names(df_fit_tf_list) == "control"),
  MoreArgs = list(betaREF_result = result_betaREF, nuREF_result = result_nuREF),
  SIMPLIFY = FALSE
...
result_theta = calc_est_J2R_theta_v4(
...
result_tip = cbind(
  ...
  calc_est_J2R_TIP_v1(
    ...
    nuREF_result = result_nuREF, nu_result = result_nu,
...
# Output contour plot 
gbda_j2r_tip=ggplot(data, aes(x = delta_t_v, y = delta_c_v, z = z_v)) +
